		...
		case st:get_value(fibo, "n") > 1 of
		  true ->		
			st:put_value({fibo, "f"},
			     	{
					int,
					st:return_function(fun () -> fibo(st:get_value(fibo, "n") -1) end, fibo, [st:get_value(fibo, "n") -1]) +
					st:return_function(fun () -> fibo(st:get_value(fibo, "n") -2) end, fibo, [st:get_value(fibo, "n") -2])
				});
		  false -> no_operation
		end
	  end
...
main(V_args) ->
    ...
    begin
      ...
      for(fun () -> st:get_value(main, "i") < st:get_value(main, "n") end,
	  fun () ->
		  st:put_value({main, "i"}, {int, st:get_value(main, "i") + 1})
	  end,
	  fun () ->
		  st:put_value({main, "a"}, {int, st:return_function(fun () -> fibo(st:get_value(main, "i")) end, fibo, [st:get_value(main, "i")])}),
