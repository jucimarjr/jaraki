		...
		case st:get_value(fibo, "n") > 1 of
		  true ->		
			st:put_value({fibo, "f"},
			     	{
					int,
					return_function(fun () -> fibo(st:get_value(fibo, "n") -1) end, fibo, [st:get_value(fibo, "n") -1]) +
					return_function(fun () -> fibo(st:get_value(fibo, "n") -2) end, fibo, [st:get_value(fibo, "n") -2])
				});
		  false -> no_operation
		end
	  end
...
main(V_args) ->
    ...
    begin
	  ...
	  fun () ->
		  st:put_value({main, "a"}, {int, return_function(fun () -> fibo(st:get_value(main, "i")) end, fibo, [st:get_value(main, "i")])}),
