...
get_erl_function_body(Line, JavaMethodBody, ArgsList) ->
	ErlangArgsList =
		[
			{var, ArgLine, list_to_atom("V_" ++ atom_to_list(ArgName))} ||
			{ArgLine, _ClassIdentifier, {argument, ArgName}} <- ArgsList
		],

	MappedErlangFun =
		fun(
			{var_declaration, {var_type,{VarLine, VarType}},{var_list, VarList}}
			) ->
				jaraki_identifier:insert_var_list(VarLine, VarType, VarList);
				(Statement) -> gen_erl_code:match_statement(Statement)
			end,
	ErlangStmtTemp = lists:map( MappedErlangFun, JavaMethodBody),
	ErlangStmt = [Element || Element <- ErlangStmtTemp,	Element =/= no_operation],
	[{clause, Line, ErlangArgsList, [], ErlangStmt}].

%%-----------------------------------------------------------------------------
%% Cria o modulo a partir do east.
create_module(ErlangModuleName, ErlangModuleBody) ->
	[ 
		{ attribute, 1, module, ErlangModuleName },
		{ attribute, 1, compile, export_all } 
	] 
	++ hd(ErlangModuleBody) ++ [ { eof, 1 } ].
