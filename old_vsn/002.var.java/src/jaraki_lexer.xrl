...
{Integer}			: {token, {int,		TokenLine, list_to_atom(TokenChars)}}.
{Float}				: {token, {float,	TokenLine, list_to_atom(TokenChars)}}.
{Double}			: {token, {double,	TokenLine, list_to_atom(TokenChars)}}.
...
{Digit}+\.{Digit}+	: {token, {real, TokenLine, list_to_float(TokenChars)}}.
